...
describe("atomic proposal approval", () => {
  ...
  it("creates order, batch, job, ack draft, event, and audit in one transaction", async () => {
    ...
    expect(result.outboundEmailId).not.toBeNull();
    ...
    await expect(
      prisma.outboundEmail.findUniqueOrThrow({ where: { id: result.outboundEmailId ?? "" } }),
  ...
  it("rolls back entirely when transfer queuing fails inside the boundary", async () => {
    ...
    await expect(prisma.outboundEmail.count()).resolves.toBe(0);
    await expect(
      prisma.proposal.findUniqueOrThrow({ where: { id: proposalId } }),
